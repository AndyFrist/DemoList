...
public class JDLockView extends View implements ILockView {
    ...
    public float getmOuterRadius() {
        return mOuterRadius;
    }

    public void setmOuterRadius(float mOuterRadius) {
        this.mOuterRadius = mOuterRadius;
    }

    public float getmInnerRadius() {
        return mInnerRadius;
    }

    public void setmInnerRadius(float mInnerRadius) {
    ...
    private void drawNoFinger(Canvas canvas) {
        ...
        mPaint.setColor(Color.parseColor("#FFBEC3CB"));
    ...
    private void drawFingerTouch(Canvas canvas) {
        mPaint.setStyle(Paint.Style.FILL);
        mPaint.setColor(Color.parseColor("#FE8839"));
        canvas.drawCircle(0, 0, mInnerRadius, mPaint);
    ...
    private void drawFingerUpUnmatched(Canvas canvas) {
        mPaint.setStyle(Paint.Style.FILL);
        mPaint.setColor(Color.parseColor("#FD3D01"));
        canvas.drawCircle(0, 0, mInnerRadius, mPaint);
        mPaint.setStyle(Paint.Style.STROKE);
        mPaint.setStrokeWidth(ConvertUtils.dp2px(getContext(), 1));
        canvas.drawCircle(0, 0, mOuterRadius, mPaint);
