package com.example.ex_xuxiaopeng002.myapplication.lock;
...
public class GestureLockLayout extends RelativeLayout {
    ...
    //手指触摸是，path颜色
    private int mFingerTouchColor = Color.parseColor("#FE8839");
    //手指抬起时,密码匹配path颜色
    private int mFingerUpMatchedColor = Color.parseColor("#FE8839");
    //手指抬起时,密码不匹配path颜色
    private int mFingerUpUnmatchedColor = Color.parseColor("#FFBEC3CB");
...
//    }
        ...
        if (lockView != null) {
            ...
            if (!mChooseList.contains(childId)) {
                ...
                if (mChooseList.size() == 1) {
                    ...
                    if (mILockView instanceof JDLockView) {
                        my = (int) (((JDLockView) mILockView).getmOuterRadius() - ((JDLockView) mILockView).getmInnerRadius());
                        mPath.lineTo(mLastPathX - my, mLastPathY - my);
                    } else
    ...
    private boolean checkPointInChild(View childView, int x, int y) {
        ...
        return x >= childView.getLeft() + padding && x <= childView.getRight() - padding
    ...
    public void setAnswer(String answer) {
        if (answer.startsWith("[") && answer.endsWith("]")) {
            ...
            for (String answer1 : answers) {
                mAnswerList.add(Integer.parseInt(answer1.trim()));
            }
        }
    }
